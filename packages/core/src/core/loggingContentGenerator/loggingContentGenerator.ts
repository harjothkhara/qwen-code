...
export class LoggingContentGenerator implements ContentGenerator {
  ...
  private safelyLogApiError(
    ...
    // A deliberate user cancel is not an API error, so skip the api_error
    // event entirely — the span already records the cancellation through its
    // aborted status, so the signal isn't lost. Without this a user cancel is
    // emitted as a `qwen-code.api_error` with error_type `APIUserAbortError`,
    // which is what #8356 reported. `isUserCancel` gates on the cancel being
    // explicitly tagged (not just any aborted signal), so an internal deadline
    // that aborts this same request stays reported.
